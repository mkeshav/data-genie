...
class QualityChecker(object):
    ...
    def _apply_has_one_of(self, node) -> Tuple[str, Any, bool]:
        ...
        ignore_case = False
        for c in node.children:
            if not isinstance(c, Tree) and c.type == 'BOOL':
                ignore_case = self._str_to_bool(c.value)
    ...
    def _parse_spec(check_spec) -> Tree:
        p = Lark(resource_string(__name__, 'data/dqc_grammar.lark').decode('utf-8'), parser='lalr')
